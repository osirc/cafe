...
$stmt = $conn->prepare("SELECT product.id, name, price, amount, stock, path FROM cart INNER JOIN product ON 
...
if ($stmt->execute()) {
    ...
    while ($row = $result -> fetch_assoc()) {
        array_push($cart,new CartJSON($row["id"],$row["name"],$row["price"],$row["amount"],$row["stock"],
                    $row["path"]));
    }
}
